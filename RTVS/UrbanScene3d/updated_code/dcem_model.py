...
class Model(nn.Module):
    ...
    def forward(self, vel, Lsx, Lsy, horizon, f12):
        ...
        vels[:,:,:,:] = 0
        vels[:,:,:,1] = 1
        f_hat = torch.cat((torch.sum(Lsx[:, :, :, :3]*vels[:, :, :, :3],-1).unsqueeze(-1) , \
                torch.sum(Lsy[:, :, :, :3]*vels[:, :, :, :3],-1).unsqueeze(-1)),-1)
        ...
        if horizon < 0:
            ...
            vel = vel * loss_norm
            vel = vel.view(6,)
            self.v_interm.append(vel.detach().cpu().numpy())
            del loss 
            del loss_norm
            del loss_fn
            del sorted, indices
        ...
        # update sigma
        mu_copy = self.mu
        self.mu = vel.clone() # mu, sigma 
        self.mu.detach()
        self.sigma = ((mu_copy - vel)**2).sqrt().detach()
        del mu_copy
