...
class Rtvs:
    ...
    def get_vel(self, img_src, pre_img_src, f12, img_name, step, mask, depth =None):
        ...
        for itr in range(self.iterations):
            mu, f_hat = self.cem.forward(vel, Lsx, Lsy, self.horizon, f12)
        ...
        vel, f_hat = self.cem.forward(vel, Lsx, Lsy, self.horizon,
...
def get_interaction_data_1(d1, ct, Cy, Cx, flowdepth=False):
    ky = Cy
    kx = Cx
    xyz = np.zeros([d1.shape[0], d1.shape[1], 3])
    Lsx = np.zeros([d1.shape[0], d1.shape[1], 6])
