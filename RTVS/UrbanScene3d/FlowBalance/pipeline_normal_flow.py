...
from rtvs_normal_flow import Rtvs
...
def save_mask(mask, step, rgb_image, img_name):
    ...
    folder = "./logs_normal/" +img_name + "/save_mask"
    folder1 = "./logs_normal/" +img_name + "/save_rgb"
...
def main():
    ...
    # client = airsim.MultirotorClienvent(ip = "10.2.36.227")
    ...
    # client.moveToPositionAsync(int(0), int(0), int(0), int(10), 3e+38, drivetrain = airsim.DrivetrainType.ForwardOnly, yaw_mode=airsim.YawMode(False, 0)).join() #, 3e+38, drivetrain = airsim.DrivetrainType.ForwardOnly, yaw_mode=airsim.YawMode(False, 0)).join()
    ...
    #     client.startRecording()
        client.simSetTraceLine([0, 0, 1, 1],   150)
        plan(pre_img_src)   
    ...
    #     pass
        ...
        with open("./logs_normal/data.txt", 'a') as f:
            print("data = ", data)
            f.writelines([json.dumps(data)])
    
def is_obstacle_there(mask):
    print(mask.sum())
    return mask.sum() > 50000
...
def plan(pre_img_src):
    ...
    folder = "./logs_normal/" + target 
    
    if not os.path.exists(folder+'/' + target):
        os.makedirs(folder+'/' + target)
    
    
    f =  open(f'./logs_normal/{target}/trajectory_normal_flow.txt', 'a')
    f.truncate(0)
    ...
    while True:
        ...
        folder = f"./logs_normal/{target}"
        if not os.path.exists(folder):
            os.makedirs(folder)
        
        
        with open(f'./logs_normal/{target}/min_depths.json', 'w') as json_file:
